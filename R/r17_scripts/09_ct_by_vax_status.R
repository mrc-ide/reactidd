...
for (round_id in 15:17) {
  ...
  df_round <- df_round %>%
    ...
    mutate(
      ...
      vax_status_noDate = factor(vax_status_noDate, levels = c(
        "Not vaccinated", "One dose", "Two dose",
        "Unknown dose", "NA"
      )),
      vax_status_noDate_v2 = factor(vax_status_noDate_v2, levels = c(
        "Not vaccinated", "One dose", "Two dose",
        "Three dose", "Unknown dose", "NA"
      ))
    ) %>%
  ...
  {
    pdf(paste0("Figures/ct_values_covid/ct_by_vax_status_r", round_id, ".pdf"),
        width = 20, height = 8)
  ...
  for (ct in c("ct1", "ct2")) {
    y <- df_round[, ct]
    mymodel <- lm(y ~ df_round$link_vax_status_booster14days)
    write.xlsx(summary(mymodel)$coefficients,
      row.names = TRUE, overwrite = TRUE,
      paste0("Figures/ct_values_covid/Linear_model_vax_status_", ct, "_r", round_id, ".xlsx")
    )
  }
}
