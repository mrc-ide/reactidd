...
nrow(df_round)
table(df_round$age%in%c("05-11", "12-17"))
table(df_round$fluvacc)
table(df_round$fluvacc, df_round$age%in%c("05-11", "12-17"))
...
for (stratum in c("all", "Yes", "No", "Unknown")){
  ...
  print(stratum)
  print(nrow(df_round))
  print(sum(df_round$age%in%c("05-11", "12-17")))
  print(sum(!df_round$age%in%c("05-11", "12-17")))
  ...
  ggvenn(list(`COVID-19`=which((df_round$estbinres==1)&(df_round$age%in%c("05-11", "12-17"))),
              `Influenza A`=which((df_round$influenzaa==1)&(df_round$age%in%c("05-11", "12-17"))),
              `Influenza B`=which((df_round$influenzab==1)&(df_round$age%in%c("05-11", "12-17")))),
         fill_color = c("royalblue", "tomato", "forestgreen"),
         stroke_color = NA,
         text_size=7,
         set_name_size=8,
