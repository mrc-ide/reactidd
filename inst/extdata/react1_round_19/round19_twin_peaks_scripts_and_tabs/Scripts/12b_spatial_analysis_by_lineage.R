...
model_folder=paste0("30km_2972nn_", gsub("\\.", "", mylineage))
...
round_dat[which(!round_dat$u_passcode%in%ids_lineage), "estbinres"]=0
round_dat[which(round_dat$u_passcode%in%ids_lineage), "estbinres"]=1
round_dat[which(round_dat$u_passcode%in%recombinant_ids), "estbinres"]=0
print(table(round_dat$estbinres, useNA = "always"))
...
if (mylineage=="BA.2"){
  high_colour <- "navy"
  max_legend <- ceiling(max(ltla_dat$ltla_prevalence) * 100) / 100
} else {
  high_colour <- "darkred"
  max_legend <- ceiling(max(ltla_dat$ltla_prevalence) * 1000) / 1000
}
...
for (region_name in c("South West", "South East", "London", "East of England")){
  print(region_name)
  ldn_map <- ggplot() +
    geom_sf(
      data = ltla_dat %>% filter(rgn19nm == region_name),
      aes(fill = ltla_prevalence), colour = NA
    ) +
    scale_fill_gradient(low = "#FEF0F0", high = "navy", limits = c(0, max_legend)) +
    theme_bw() +
    xlab("") +
    ylab("") +
    labs(fill = "prevalence") +
  ...
  ggsave(
    plot = ldn_map,
    filename = paste0(
      save_dir, dist, "km_nn_",
      gsub(" ", "_", tolower(region_name)),
      "_map_r", round_id, ".pdf"
    ),
    device = "pdf",
    height = 10,
...
if (direct_export) {
  file.copy(
    from = paste0(save_dir, dist, "km_ordered_prev.csv"),
    to = "T:/", overwrite = TRUE
  )
  myfiles <- list.files(save_dir)
  myfiles <- myfiles[grep("pdf", myfiles)]
  file.copy(
    from = paste0(save_dir, myfiles),
    to = "T:/", overwrite = TRUE
  )
}
