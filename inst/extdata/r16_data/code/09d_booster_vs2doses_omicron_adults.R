...
lag=14
# Path to output file
output_file <- paste0("E:/Group/report/round16/Tables/OR_Booster_efficiency_2Dose_14_omicron_",lag)
...
for (id in 1:length(round_ids)) {
  ...
  if(round_id==16){
    ...
    if(lag==14){
      ...
      filter(vax_status %in% c("2 doses", "3 doses")) %>%
      filter((estbinres == 0) | (estbinres == 1 & u_passcode %in% ids_omicron))
    }
    if(lag==7){
      df_round <- df_round %>%
        mutate(
...
for (model_id in paste0("m", 0:2)) {
  ...
  print(nrow(mydata))
  if(model_id=="m0"){
    mytable <- c(table(full_dataset$vax_status,full_dataset$estbinres)[1,],"-")
  }
