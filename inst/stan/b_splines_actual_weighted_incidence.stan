...
data {
  ...
  int num_days;             // number of days incidence is estimated for (range data + 50 )
  ...
  real X_data[num_data];
  real X[num_days];
  matrix[num_days, num_data] conversion_matrix; 	//This matrix converts from incidence to prevalence
  real sens; 						// The sensitivity of the test
}
...
transformed parameters {
  ...
  row_vector[num_days] incidence;
  vector[num_data] Y_hat;
  a[1] = a_raw[1];
  a[2] = a_raw[2];
  for (i in 3:num_basis)
    a[i] = 2*a[i-1] - a[i-2] + a_raw[i]*tau;
  incidence =  inv_logit(to_row_vector(a*B));

  // Convert incidence to Prevalence
  Y_hat = to_vector(incidence*conversion_matrix*sens);
